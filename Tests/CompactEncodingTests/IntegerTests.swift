...
@Test func testUInt8() throws {
  ...
  UInt8.preencode(&state, 12)
  state.allocate()

  try UInt8.encode(&state, 12)
...
@Test func testUInt16() throws {
  ...
  UInt16.preencode(&state, 12 | (34 << 8))
  state.allocate()

  try UInt16.encode(&state, 12 | (34 << 8))
  #expect(state.buffer[0] == 12)
  #expect(state.buffer[1] == 34)

  state.rewind()

  let value = try UInt16.decode(&state)
...
@Test func testUInt32() throws {
  ...
  UInt32.preencode(&state, 12 | (34 << 8) | (56 << 16) | (78 << 24))
  state.allocate()

  try UInt32.encode(&state, 12 | (34 << 8) | (56 << 16) | (78 << 24))
  #expect(state.buffer[0] == 12)
  #expect(state.buffer[1] == 34)
  ...
  let value = try UInt32.decode(&state)
